-- Drop and recreate the cleanup function to include both pending and processing deposits
...
BEGIN
  ...
  FOR v_record IN
    ...
    WHERE t.type = 'deposit'
      AND t.status IN ('pending', 'processing')  -- Include both statuses
  ...
  LOOP
    ...
    VALUES (
      ...
      'deposit_rejected',
      '❌ Deposit Rejected',
      'Your deposit request of ₹' || v_record.amount || ' was rejected due to no action for 5+ hours. Please try again or contact support.'
    );
