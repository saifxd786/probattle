...
// Track coordinates for each color - MUST match LudoBoard.tsx exactly!
// Each color starts from their respective starting position and goes around the board
const COLOR_TRACK_COORDS: { [color: string]: { x: number; y: number }[] } = {
  // Red (top-left quadrant) - starts from bottom going up, then clockwise
  red: [
  ...
  // Green (top-right quadrant) - starts from left going right, then clockwise
  green: [
  ...
  // Yellow (bottom-right quadrant) - starts from top going down, then clockwise
  yellow: [
  ...
  // Blue (bottom-left quadrant) - starts from right going left, then clockwise
  blue: [
...
const SAFE_POSITIONS = [
  ...
  { x: 6.5, y: 13.5 },   // Red start
  { x: 1.5, y: 6.5 },    // Green start
  { x: 8.5, y: 1.5 },    // Yellow start
  { x: 13.5, y: 8.5 },   // Blue start
  // Safe spots (star positions on the board)
  { x: 6.5, y: 2.5 },
  { x: 2.5, y: 6.5 },
  { x: 12.5, y: 8.5 },
  { x: 8.5, y: 12.5 },
